...
test('auth sync gates app data before sign in and moves account controls into the more-page account menu', () => {
  ...
  assert.match(source, /toggleAccountMenu/)
...
test('sign out clears local account data before returning to auth gate', () => {
  ...
  assert.match(source, /appStorage\(\)\?\.reset\?\.\(\)/)
...
test('auth restore waits for app storage bridge before creating or restoring vault data', () => {
  const source = fs.readFileSync(path.join(root, 'auth_sync.js'), 'utf8')
  const storageSource = fs.readFileSync(path.join(root, 'storage_v2.js'), 'utf8')

  assert.match(source, /function appStorage\(\)/)
  ...
  assert.match(source, /root\.MTStorage/)
  assert.match(storageSource, /window\.MTStorage = Storage/)
  assert.doesNotMatch(source, /root\.Storage\?\.buildExportPayload/)
